  ...
  class << self
    ...
    def request_uri(options = nil)
      ...
      if (options.to_s =~ /\A[+-]?\d+\Z/)
        uri = URI("#{ServiceHelper.uri_for('centre', protocol = 'http', host: :external)}/notices/#{options}.json")
      else
        # options was not numeric
        uri = URI("#{ServiceHelper.uri_for('centre', protocol = 'http', host: :external)}/notices.json")
