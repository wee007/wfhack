  ...
  class << self
    ...
    def request_uri(options=nil)
      if options.is_a?(Fixnum) || options.is_a?(String)
        URI("#{ServiceHelper.uri_for('canned-search', protocol = 'https', host: :external)}/curations/#{options}.json")
      else
        uri = URI("#{ServiceHelper.uri_for('canned-search', protocol = 'https', host: :external)}/curations.json")
